...
document.querySelectorAll(".nav-btn").forEach(btn => {
  btn.addEventListener("click", () => {
    ...
    // When switching to analytics, reset heatmap so it measures the now-visible container
    if (view === "analytics" && allEntries.length) {
      heatmapCells = [];
      renderAll();
    }
  });
...
stopBtn.onclick = async () => {
  ...
  if (elapsedSeconds < 60) {
    timerStatus.textContent = "Too short";
...
async function loadEntries() {
  ...
  buildTagPills();
  // If analytics is already visible (e.g. page refresh), reset heatmap so it measures correctly
...
function buildTagPills() {
  allKnownTags = [...new Set(allEntries.map(e => e.tag).filter(Boolean))].sort();
...
function renderTagPills() {
  ...
  allPill.className = "tag-pill" + (selectedTag === "__all__" ? " active" : "");
  allPill.textContent = "All";
  allPill.dataset.tag = "__all__";
  allPill.onclick = () => selectTag("__all__");
  container.appendChild(allPill);

  // Individual visible tag pills
  const visibleTags = allKnownTags.filter(t => !hiddenTags.has(t));
  ...
  // If selected tag was hidden, fall back
  if (selectedTag !== "__all__" && hiddenTags.has(selectedTag)) {
    selectTag(visibleTags[0] || "__all__");
  }
}
...
document.getElementById("tagFilterBtn").onclick = () => {
  ...
  allKnownTags.forEach(tag => {
    ...
    const checked = !hiddenTags.has(tag);
    item.innerHTML = `
      <span>
    ...
    item.querySelector("input").onchange = e => {
      if (e.target.checked) hiddenTags.delete(tag);
...
document.getElementById("closeTagFilterBtn").onclick = () => {
  document.getElementById("tagFilterModal").classList.add("hidden");
};
document.getElementById("tagFilterModal").addEventListener("click", e => {
  if (e.target === document.getElementById("tagFilterModal"))
    document.getElementById("tagFilterModal").classList.add("hidden");
});
...
const prevWeek   = allEntries.filter(e => { const d = e.timestamp.toDate(); return d >= prevCutoff && d < prevWeekEnd; });
  ...
  // — Total all time —
  const allTotal = allEntries.reduce((s, e) => s + e.seconds, 0);
  document.getElementById("cardTotalAllTime").textContent = fmtHours(allTotal);
  document.getElementById("cardTotalAllTimeSub").textContent = `${allEntries.length} session${allEntries.length !== 1 ? "s" : ""} recorded`;
...
function renderDailyBar() {
  ...
  charts.daily = new Chart(document.getElementById("dailyBarChart"), {
    type: "bar",
    data: { labels, datasets: [{ data, backgroundColor: bgs, borderRadius: 6, borderSkipped: "bottom" }] },
    options: {
...
function renderTagCharts() {
  ...
  const isAll = selectedTag === "__all__";
  const label = isAll ? "All tags" : selectedTag;
  document.getElementById("tagContextName").textContent = label;
  document.getElementById("tagBarTitle").textContent    = isAll ? "Time by class — all tags" : `Time by class — ${selectedTag}`;
  document.getElementById("tagLineTitle").textContent   = isAll ? "All tags over time — weekly totals" : `${selectedTag} over time — weekly totals`;

  const rangeEntries = entriesInRange(allEntries, selectedRange);
  const tagEntries   = isAll ? rangeEntries : rangeEntries.filter(e => e.tag === selectedTag);

  // Always destroy & recreate tag charts so switching tags/All never reuses stale chart state
  destroyChart("tagBar");
  destroyChart("dist");
  destroyChart("tagLine");
...
function renderTagBar(tagEntries) {
  ...
  const isAll = selectedTag === "__all__";
  const sorted = Object.entries(totals).sort((a,b) => b[1]-a[1]).slice(0, isAll ? 5 : 20);
...
function renderTagLine(tagEntries) {
  // Always use all-time entries for the trend line (range filter collapses to 1 week = flat line)
  const isAll = selectedTag === "__all__";
  const allTimeEntries = isAll
    ? allEntries
    : allEntries.filter(e => e.tag === selectedTag);
  ...
  // Fill gaps — parse keys as local time to avoid UTC date-shift
  if (sorted.length > 1) {
    const filled = [];
    const parseLocal = k => { const [y,m,d] = k.split("-").map(Number); return new Date(y, m-1, d); };
    const cur = parseLocal(sorted[0][0]);
    const last = parseLocal(sorted[sorted.length-1][0]);
    while (cur <= last) {
  ...
  const labels = sorted.map(([k]) => {
    // Parse as local time to avoid UTC-offset making dates appear in the future
  ...
  charts.tagLine = new Chart(document.getElementById("tagLineChart"), {
    ...
    options: {
      ...
      scales: {
        ...
        y: { ...SCALE_Y, min: 0, ticks: { color: "#9a9a9a", callback: v => v + "h" } }
      }
    }
  });
}
...
function generateHeatmap(days) {
  ...
  const CELL_GAP    = 3;
  const DAY_LABEL_W = 30; // px reserved for Mon/Wed/Fri labels
  ...
  // Compute cell size to fill available container width (no scroll)
  const wrapEl     = document.querySelector(".heatmap-wrap");
  const available  = (wrapEl ? wrapEl.clientWidth : 900) - DAY_LABEL_W - CELL_GAP;
  const rawCell    = (available - (totalCols - 1) * CELL_GAP) / totalCols;
  const CELL_SIZE  = Math.max(11, Math.min(18, Math.floor(rawCell)));
  const CELL_STEP  = CELL_SIZE + CELL_GAP;

  // Update CSS row heights to match computed cell size
  grid.style.gridTemplateRows     = `repeat(7, ${CELL_SIZE}px)`;
  document.querySelector(".heatmap-day-labels").style.gridTemplateRows = `repeat(7, ${CELL_SIZE}px)`;

  // Month labels — emit on 1st of month, skip if too close to previous
  const monthLabels = [];
  let lastLabelCol  = -3;
  for (let i = 0; i < days; i++) {
    ...
    if (d.getDate() === 1 || i === 0) {
      ...
      if (col - lastLabelCol >= 2) {
        monthLabels.push({ col, text: d.toLocaleString("default", { month: "short" }) });
        lastLabelCol = col;
      }
    }
  }

  monthsEl.style.gridTemplateColumns = `repeat(${totalCols}, ${CELL_STEP}px)`;
